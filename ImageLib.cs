...
namespace ImageLabel
{
    class Box
    {
        ...
        public Point p1, p2;
        public string plate;
        public Box(string img, string label, Rectangle rect,string plate)
        {
            this.img = img;
            this.label = label;
        ...
        public Box(string img, string label, Point upleft, Point bottomright,string plate)
        {
            this.img = img;
            this.label = label;
            p1 = upleft;
    ...
    class ImageLib
    {
        ...
        public Dictionary<string, string> plateDict = new Dictionary<string, string>();
        ...
        public void AddLabel(string key, string label, Point p1, Point p2)
        {
            ...
            boxDict[key].Add(new Box(key, label, p1, p2, null));
        }
        ...
        #endregion
        public void AddPlate(string key ,int index,string value)
        {
            boxDict.TryGetValue(key, out List<Box> boxList);
            boxList[index].plate = value;
        ...
        {
            if (plateFilePath is null) return;

            destLabelFilePath = plateFilePath;
            using (StreamWriter sw = new StreamWriter(destLabelFilePath))
            {
                foreach (string imgName in imgNameList)
                {
                    ...
                    foreach (Box box in boxList)
                    {
                        if (box.plate == null || box.plate == "")
                            continue;
                        else 
                        {
                            sw.WriteLine(box.ToString()+' '+box.plate);
                        }
